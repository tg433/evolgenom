...
function main()
	...
	Random.seed!(seed_normal)					# random number generator initialization for drawing sequencing reads
	rng_error = MersenneTwister(seed_error)		# setting up a different rnd generator for generating sequencing errors

		# defining the distribution of total read counts. It can be either given in an input file, or defined as uniform.
	...
	read_tree(treefile, branchlengths, clonal_peak, mu)		# reading the lineage tree

	for nrun in 1:n_samples						# cycle over different generated samples
		mutations, total_reads_hist = Dict{Int64, Dict{Tuple{Int64, Int64, Int64}, Int64}}(), Dict{Int64, Int64}()
	
		n_observed_all_mutations, n_observed_true_mutations = simulate_data(n_sites, branchlengths, mu, seqdepthdistr, errorrate, mutations, rng_error, total_reads_hist, logerr, population_size, ploidy, contamination)	# getting the simulated reads
...
function read_empiricalcdf(cdffile::String, seqdepthdistr::EmpiricalCDF{Float64})
		...
		# reading the distribution of total read counts from a file
...
function read_empiricalcdf(seq_depth::Int64, seqdepthdistr::EmpiricalCDF{Float64})
		...
		# using an uniform total read count distribution, defined by seq_depth
...
function read_tree(treefile::String, branchlengths::Dict{Rational, Float64}, clonal_peak::Int64, mu::Float64)
	...
	for i in 1:size(treedata, 1)
		...
		branch = numerator // denominator				# equivalent to f_k in the manuscript's terms; stored as a rational number
		if haskey(branchlengths, branch)
			branchlengths[branch] += treedata[i,3]		# note that different branches having the same number of leaves are grouped together
		else
...
function simulate_data(n_sites::Int64, branchlengths::Dict{Rational, Float64}, mu::Float64, seqdepthdistr::EmpiricalCDF{Float64}, errorrate::Float64, mutations::Dict{Int64, Dict{Tuple{Int64, Int64, Int64}, Int64}}, rng_error::MersenneTwister, total_reads_hist::Dict{Int64, Int64}, logerr::Int64, population_size::Int64, ploidy::Int64, contamination::Float64)
	...
	for site in 1:n_sites		# loop over sites
	
			# drawing the total read count of the current site, using the total read count distribution defined earlier
		...
		if haskey(total_reads_hist, total_reads)
			...
			# deciding whether the current site is mutant, if yes, which branch of the lineage tree is affected
		...
		for branch in keys(branchlengths)
			...
			# drawing mutant and wild type read counts (sequencing errors will be considered later)
		
		if site_is_mutant_flag
			successes = round(Int64, population_size * selected_branch)		# number of mutant DNAs. There are population_size tumour cells in the sequenced sample.
			failures = round(Int64, population_size * ploidy / (1.0 - contamination)) - successes	# number of wild type DNAs
			true_mutant_reads = rand(Hypergeometric(successes, failures, total_reads))	# drawing the number of mutant and wild type reads from the given number of mutant and wild type DNAs flying around
		else
			true_mutant_reads = 0
		end

			# let's take into account sequencing errors
...
main()		# starting the function main
